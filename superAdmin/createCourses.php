...
if(isset($_POST['submit'])){
    ...
    // Checks the Course ID
    $quer=mysqli_query($con,"select * from tblcourse where courseId ='$courseId'");
    $retr=mysqli_fetch_array($quer);
    //Checks the Course Title
    $query=mysqli_query($con,"select * from tblcourse where courseTitle ='$courseTitle' and departmentId ='$departmentId'");
    $ret=mysqli_fetch_array($query);

    
    if($courseId == 0000){ //check the courseID

      $alertStyle ="alert alert-danger";
      $statusMsg="This Course ID cannot be used!";

    }

    else if($retr > 0){ //check the courseID

      $alertStyle ="alert alert-danger";
      $statusMsg="This Course ID has already been used!";

    }

    else if($ret > 0){ //Check the coure Title
      ...
      $statusMsg="This Course already exist in this department!";
        ...
        $query=mysqli_query($con,"insert into tblcourse(courseId,courseTitle,courseCode,courseUnit,facultyId,departmentId,staffId,dateAdded) value('$courseId','$courseTitle','$courseCode','$courseUnit','$facultyId','$departmentId','$staffId','$dateAdded')");
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-6">
                                                    <label for="x_card_code" class="control-label mb-1">ID</label>
                                                        <input id="" maxlength="4" onkeypress="return isNumber(event)" name="courseId" type="text" class="form-control cc-cvc" Required data-val="true" data-val-required="Please enter the security code" data-val-cc-cvc="Please enter a valid security code" placeholder="ID should start from 0001">
                                                        </div>
                                                    </div>
                                                    <div>
                                                <div class="row">
                ...
                <br><br>
                    <div class="col-md-12">
                        <div class="card">
                            ...
                            <?php
        $ret=mysqli_query($con,"SELECT tblstaff.staffId, tblstaff.firstName, tblstaff.lastName,tblcourse.courseId,tblcourse.courseTitle,tblcourse.courseCode,tblcourse.dateAdded,
       tblcourse.courseUnit, tblfaculty.facultyName,tbldepartment.departmentName
        from tblcourse 
        ...
        while ($row=mysqli_fetch_array($ret)) {
                ...
                <td><a href="editCourses.php?editCourseId=<?php echo $row['courseId'];?>" title="Edit Details"><i class="fa fa-edit fa-1x"></i></a></td>
                <td><a onclick="return confirm('Are you sure you want to delete?')" href="deleteCourse.php?delid=<?php echo $row['courseId'];?>" title="Delete Course"><i class="fa fa-trash fa-1x"></i></a></td>
                </tr>
                <?php 
                $cnt=$cnt+1;
                }?>
